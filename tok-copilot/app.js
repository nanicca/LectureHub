...
function renderSession() {
  ...
  (c.섹션 || []).forEach((s, i) => {
    ...
    if (s.type === "문단") {
      ...
      // 프롬프트는 .prompts 안에서 나란히(병렬) 배치됩니다. 화면이 좁으면 자동 1열.
  ...
  document.querySelectorAll(".copy-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const 알림 = (ok) => {
        const old = "📋 복사";
        btn.textContent = ok ? "✓ 복사됨" : "✗ 직접 복사해 주세요";
      ...
      // 회사 PC 등에서 클립보드가 막혀 있으면 예전 방식으로 한 번 더 시도합니다.
      const 예전방식 = () => {
        try {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(() => 알림(true), 예전방식);
      } else {
        예전방식();
      }
    });
