...
function renderSession() {
  ...
  document.getElementById("세션히어로").innerHTML = `
    <img class="brand-logo ablearn" src="logo-ablearn.png" alt="ABLEARN" />
    <div class="wrap">
      <div class="brand-row">
        <img class="brand-logo toda" src="logo-toda.png" alt="TODA materials" />
        <span class="date">${esc(d.강의일자)}</span>
      </div>
  ...
  (c.섹션 || []).forEach((s, i) => {
    ...
    } else if (s.type === "파일다운로드") {
      if (s.설명) html += `<p>${esc(s.설명)}</p>`;
      const fname = (s.파일명 || "").split("/").pop();
      html += `<a class="file-download" href="${esc(s.파일명)}" download target="_blank" rel="noopener">
        <span class="ico">📎</span>
        <span class="fd-text">
          <span class="fd-name">${esc(s.표시이름 || fname)}</span>
          ${s.부연설명 ? `<span class="fd-desc">${esc(s.부연설명)}</span>` : ""}
        </span>
        <span class="btn">${esc(s.버튼 || "다운로드")}</span>
      </a>`;
